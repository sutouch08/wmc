...
function pagination_config( $base_url, $total_rows = 0, $perpage = 20, $segment = 3)
{
	$ci =& get_instance();
	$rows = get_rows();
	$input_rows  = '<p class="text-center font-size-11" style="margin:0; padding:5px 0;">';
	$input_rows .= 'ทั้งหมด '.number($total_rows).' รายการ';
	$input_rows .= '<input type="number" name="set_rows" id="set_rows" class="input-mini text-center margin-left-10 margin-right-10" style="border-radius:5px !important;" value="'.$rows.'" />';
	$input_rows .= 'ต่อหน้า ';
	$input_rows .= '<buton class="btn btn-success btn-mini margin-left-5" type="button" onClick="set_rows()">แสดง</button>';
	$input_rows .= '</p>';

	$ci->result_items = $input_rows;
